...
template<typename morton, typename coord> __device__ __host__ inline morton m3D_e_sLUT(const coord x, const coord y, const coord z);
template<typename morton, typename coord> __device__ __host__ inline morton m3D_e_sLUT_ET(const coord x, const coord y, const coord z);
template<typename morton, typename coord> __device__ __host__ inline morton m3D_e_LUT(const coord x, const coord y, const coord z);
template<typename morton, typename coord> __device__ __host__ inline morton m3D_e_LUT_ET(const coord x, const coord y, const coord z);
template<typename morton, typename coord> __device__ __host__ inline morton m3D_e_magicbits(const coord x, const coord y, const coord z);
template<typename morton, typename coord> __device__ __host__ inline morton m3D_e_for(const coord x, const coord y, const coord z);
template<typename morton, typename coord> __device__ __host__ inline morton m3D_e_for_ET(const coord x, const coord y, const coord z);

// AVAILABLE METHODS FOR DECODING
template<typename morton, typename coord> __device__ __host__ inline void m3D_d_sLUT(const morton m, coord& x, coord& y, coord& z);
template<typename morton, typename coord> __device__ __host__ inline void m3D_d_sLUT_ET(const morton m, coord& x, coord& y, coord& z);
template<typename morton, typename coord> __device__ __host__ inline void m3D_d_LUT(const morton m, coord& x, coord& y, coord& z);
template<typename morton, typename coord> __device__ __host__ inline void m3D_d_LUT_ET(const morton m, coord& x, coord& y, coord& z);
template<typename morton, typename coord> __device__ __host__ inline void m3D_d_magicbits(const morton m, coord& x, coord& y, coord& z);
template<typename morton, typename coord> __device__ __host__ inline void m3D_d_for(const morton m, coord& x, coord& y, coord& z);
template<typename morton, typename coord> __device__ __host__ inline void m3D_d_for_ET(const morton m, coord& x, coord& y, coord& z);

// ENCODE 3D Morton code : Pre-Shifted LookUpTable (sLUT)
template<typename morton, typename coord>
__device__ __host__ inline morton m3D_e_sLUT(const coord x, const coord y, const coord z) {
	morton answer = 0;
...
template<typename morton, typename coord>
__device__ __host__ inline morton m3D_e_LUT(const coord x, const coord y, const coord z) {
	morton answer = 0;
	for (unsigned int i = sizeof(coord); i > 0; --i) {
		unsigned int shift = (i - 1) * 8;
		answer =
...
template<typename morton, typename coord>
__device__ __host__ inline morton compute3D_ET_LUT_encode(const coord c, const coord *LUT) {
...
template<typename morton, typename coord>
__device__ __host__ inline morton m3D_e_sLUT_ET(const coord x, const coord y, const coord z) {
...
// Figuring this out is probably too costly in most cases.
template<typename morton, typename coord>
__device__ __host__ inline morton m3D_e_LUT_ET(const coord x, const coord y, const coord z) {
	morton answer_x = compute3D_ET_LUT_encode<morton, coord>(x, Morton3D_encode_x_256);
...
template<typename morton, typename coord>
__device__ __host__ static inline morton morton3D_SplitBy3bits(const coord a) {
...
template<typename morton, typename coord>
__device__ __host__ inline morton m3D_e_magicbits(const coord x, const coord y, const coord z){
...
template<typename morton, typename coord>
__device__ __host__ inline morton m3D_e_for(const coord x, const coord y, const coord z){
	morton answer = 0;
...
template<typename morton, typename coord>
__device__ __host__ inline morton m3D_e_for_ET(const coord x, const coord y, const coord z) {
	morton answer = 0;
...
template<typename morton, typename coord>
__device__ __host__ inline coord morton3D_DecodeCoord_LUT256(const morton m, const uint_fast8_t *LUT, const unsigned int startshift) {
	morton a = 0;
...
template<typename morton, typename coord>
__device__ __host__ inline void m3D_d_sLUT(const morton m, coord& x, coord& y, coord& z) {
...
template<typename morton, typename coord>
__device__ __host__ inline void m3D_d_LUT(const morton m, coord& x, coord& y, coord& z) {
	x = morton3D_DecodeCoord_LUT256<morton, coord>(m, Morton3D_decode_x_512, 0);
...
template<typename morton, typename coord>
__device__ __host__ inline void m3D_d_sLUT_ET(const morton m, coord& x, coord& y, coord& z){
	x = 0; y = 0; z = 0;
...
template<typename morton, typename coord>
__device__ __host__ inline void m3D_d_LUT_ET(const morton m, coord& x, coord& y, coord& z){
	x = 0; y = 0; z = 0;
	unsigned long firstbit_location = 0;
...
template<typename morton, typename coord>
__device__ __host__ static inline coord morton3D_GetThirdBits(const morton m) {
...
template<typename morton, typename coord>
__device__ __host__ inline void m3D_d_magicbits(const morton m, coord& x, coord& y, coord& z){
...
template<typename morton, typename coord>
__device__ __host__ inline void m3D_d_for(const morton m, coord& x, coord& y, coord& z){
	x = 0; y = 0; z = 0;
	unsigned int checkbits = static_cast<unsigned int>(floor((sizeof(morton) * 8.0f / 3.0f)));
	for (unsigned int i = 0; i <= checkbits; ++i) {
		morton selector = 1;
...
template<typename morton, typename coord>
__device__ __host__ inline void m3D_d_for_ET(const morton m, coord& x, coord& y, coord& z) {
	x = 0; y = 0; z = 0;
	unsigned long firstbit_location = 0;
	if(!findFirstSetBit<morton>(m, &firstbit_location)) return;
